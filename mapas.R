...
library(foreign)
#library(sf)    <-- ta dando erro sla pq
library(tidyr)
library(geobr)
library(ggplot2)
library(dplyr)
...
setwd("C:/Users/mn_sa/Documents/tcc")
...
dados_grau_part <- read.csv("C:/Users/mn_sa/Documents/tcc/grau_part.csv")
...
# 
cap_pag_div <- read.csv("C:/Users/mn_sa/Documents/tcc/cap_pag_div.csv")
...
divida_lrf_dados <- read.csv("C:/Users/mn_sa/Documents/tcc/divida_lrf.csv")
...
cons_inter <- read.csv("C:/Users/mn_sa/Documents/tcc/cons_inter.csv")

cons_inter <- cons_inter |> 
        select(code_muni, eq1) |> 
         rename(eq1_cons_inter = eq1) #talvez essas linhas estejam erradas

cons_inter_geo <- left_join(muni_br,cons_inter, by = "code_muni")


cons_inter_plot <- ggplot(cons_inter_geo)+
        geom_sf(aes(fill = eq1, col = eq1, geometry = geom)) +
        scale_color_gradientn(colours = brewer.pal(9, 'RdYlGn'))+
        scale_fill_gradientn(colours = brewer.pal(9, 'RdYlGn' ))


###########---------------------prpcapita sem limite (nao estou utilizando este---------------------#################

#prpcapita <- read.csv("C:/Users/mn_sa/Documents/tcc/prcapita.csv")
...
#  mutate(eq1_prpcapita2 = (((prpc - min(prpc)) / (max(prpc) - min(prpc))) * 5) + 1) # |>
 # # rename(code_muni = codigo) # ta dando pau aqui
...
cap_ger <- read.csv("C:/Users/mn_sa/Documents/tcc/cap_ger.csv")
